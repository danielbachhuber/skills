...
Work a reviewer's comments one at a time. Create a worktree if you need to, so the work isn't done in the main checkout. You might already be on a worktree, though, so creating one might not be necessary. Each comment gets its own diff, the user's approval, and its own commit. Push once at the end, then reply to each addressed comment with the commit SHA and nothing else.
...
One commit per comment. Narrowest accurate scope, no issue references. Body says what the reviewer found and what changed.

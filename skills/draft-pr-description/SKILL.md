...
Find out whether the repo has end-to-end tests, because the testing instructions depend on
it:

```bash
ls playwright.config.* cypress.config.* */playwright.config.* */cypress.config.* 2>/dev/null
ls -d e2e* */e2e* 2>/dev/null
ls .github/workflows/ 2>/dev/null | grep -i -E 'e2e|playwright|cypress'
```

If there is a suite, read the specs that touch what you changed, and decide whether each
one actually asserts the new behavior. Run the ones that do, with the repo's own command.
Also read the workflow's trigger conditions, such as draft PRs, bot authors, and path
filters, so you know whether CI will run the suite on this PR at all.

## 2. Choose the structure

The default structure is the one in `writing.md`: Proposed Changes, Approach, History,
Decisions, Testing Instructions. Use it even when the repo has a template of its own.

Still look for the repo's format document, template, and PR-description skill, and read
what you find:
...
They decide two things. First, whether there is a reason to use the repo's structure
instead. The usual one is a stack of PRs whose other descriptions already follow the
repo's format, where matching them keeps the series readable. Another is a check or bot
that fails a PR missing the template's headings. When you switch to the repo's structure,
say why when you show the draft.

Second, the repo's rules that are not about structure still apply inside the default
sections: how to write testing steps, what to link, and what to leave out.
...
- **Coverage claimed from a spec's name.** A spec counts as E2E coverage only if it asserts
  the behavior and you ran it. Otherwise the behavior goes under what still needs a manual
  pass.
...
Relay the title, the body, and anything you flagged. If the testing instructions list a
behavior that no end-to-end spec covers but one could, offer to add the spec. Then wait.
...
| Using the repo's template by default | The default is Proposed Changes, Approach, History, Decisions, Testing Instructions. Switch to the repo's structure only for a reason, such as a stack of PRs that already uses it, and say which reason. |
| Skipping the format lookup | The repo's rules still apply inside the default sections, and they are how you find out whether there is a reason to switch. |
...
| Testing instructions that list CI checks | Say how to see the change work. Manual steps first, then, where the repo has end-to-end tests, what the specs cover. |
| Naming a spec without saying what it checks | Describe the states it sets up, the actions it takes, and what it asserts, so the reviewer knows what they can skip. |
| E2E coverage claimed from a spec's name | Read the spec, confirm it asserts the behavior, and run it. Otherwise the behavior needs a manual step. |
